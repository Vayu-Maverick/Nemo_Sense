...
>
> The repository includes complete wiring diagrams, circuit schematics, and power-distribution drawings that are critical for safe assembly and testing. If you're building or reviewing the hardware, start here:
>
...
> **285 million people** worldwide are visually impaired. Navigating a busy Indian city — with vehicles parked on footpaths, open manholes, stray animals, and uneven roads — is a daily challenge. We wanted to build something that a blind person could use independently, without expensive guide dogs or cloud-dependent wearables.
...
NEMO_SENSE uses **three independent battery units** in a deliberate redundancy design. The key insight is that BAT-1 and BAT-2 are **completely identical** — same type, same voltage, same capacity — and they feed the same power rail via a passive Schottky diode OR circuit. BAT-3 is a tiny separate 3.7V emergency rail for the buzzer and Bluetooth module, so the user always gets feedback even if the main batteries die.
...
Regular diodes have a 0.6–0.7V forward voltage drop, which means the 7.4V battery only delivers ~6.7V to the rail — borderline for reliable motor operation. Schottky diodes drop only ~0.3V, so the rail stays at a healthy 7.1V even under load.
